...
in {
  ...
  config = mkIf cfg.enable {
    ...
    # NEXT: wrap into own nix module
    # Notify me when timewarrior is tracking
    ...
    systemd.services."timew_track" = {
      script = ''
          ...
          export DISPLAY=:0
          $(${pkgs.timewarrior}/bin/timew|grep Tracking && ${pkgs.libnotify}/bin/notify-send --urgency=low --expire-time=1000 "Timewarrior" "Tracking is active") || exit 0
      '';
      serviceConfig = {
        Type = "oneshot";
